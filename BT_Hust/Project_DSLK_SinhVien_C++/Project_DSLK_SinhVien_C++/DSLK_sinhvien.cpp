...
int main() {
	...
	SinhVien sv6(23789, "Nguyen Thi Mai Thuy", "ET03");
	List.InsertFirst(sv1);
	List.InsertLast(sv2);
	List.InsertFirst(sv3);
	List.InsertFirst(sv4);
	List.InsertFirst(sv6);
	List.InsertLast(sv5);
	List.Print();
	cout << "Tim kiem sinh vien :"<<endl;
	NodeData<SinhVien>* pTmp = List.SearchNode(sv5);
	cout << pTmp->get_data().get_ma() << "    " << pTmp->get_data().get_name() << "     " << pTmp->get_data().get_lop()<< endl;//xuat data
