//1.Cai dat dslk don voi cac thao tac da hoc moi node trong danh sach chi chua 1 kieu du lieu thuoc kieu int.Viet chuong trinh sd ca C va C++
#include<iostream>
...
Node *InsertAfter(SingleList &list, Node *pNode, int k){
...
Node *InsertBefore(SingleList &list,Node *pNode,int k){
	Node *temp=CreateNode(k);
	if(list.pHead==NULL){
		list.pHead=pNode;
	}
...
int main(){
	...
	cout<<endl;
	InsertAfter(list,SearchNode(list,4),113);
	PrintList(list);
	cout<<endl;
	InsertBefore(list,SearchNode(list,2),111);
	PrintList(list);
